...
const isOlderValidator = (control: AbstractControl): ValidationErrors | null => {
	if (!control.value) {
		return null;
	}
...
export const Validations: ValidationConfig = {
	...
	'aux-depot': {
		name: [Validators.required, Validators.minLength(Consts.THREE), Validators.maxLength(Consts.FIFTY)],
		description: [Validators.maxLength(Consts.ONE_HUNDRED_TWENTY), Validators.minLength(Consts.FIVE), Validators.required],
		'id number': [Validators.required, Validators.pattern(Consts.NUMBERS_REGEX)],
		'phone number': [Validators.required, Validators.pattern(Consts.PHONE_NUMBER_REGEX)],
